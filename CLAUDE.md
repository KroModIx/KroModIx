...
- **`IDescriptionParser`-Contract** (Contracts v1.19.0): `string ToPlainText(html)` (HTML+BBCode-Mix zu Plain-Text) + `IReadOnlyList<InlineImage> ExtractImages(html)` (BBCode `[img …]URL[/img]` + HTML `<img src=…>`, dokumenten-position-sortiert). Analog `IImageDecoder`-Muster (v1.18) — Cross-Cutting-Concern-Baukasten, gemeinsame Bug-Fixes an einer Stelle. `NullDescriptionParser`-Default fuer Hosts < v1.20 (Passthrough).
