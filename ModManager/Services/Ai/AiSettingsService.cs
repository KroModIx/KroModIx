...
public sealed class AiSettingsService
{
    ...
    private readonly ISecretProtection _secret;
    private AiSettings _current;

    public AiSettingsService(ISecretProtection secret) : this(secret, DefaultConfigPath()) { }

    public AiSettingsService(ISecretProtection secret, string configPath)
    {
        _secret = secret;
    ...
    private PersistedSettings ToPersisted(AiSettings s) => new(
        Provider: s.Provider,
    ...
    private AiSettings FromPersisted(PersistedSettings p) => new(
        Provider: p.Provider,
    ...
    private PersistedConfig ToPersisted(AiProviderConfig c) =>
        new(c.Endpoint, c.Model, _secret.Protect(c.ApiKey));

    private AiProviderConfig FromPersisted(PersistedConfig? c, AiProviderType fallbackType)
    {
        ...
        return new AiProviderConfig(
            ...
            ApiKey: _secret.Unprotect(c.ApiKeyProtected));
    }
    ...
    private static string DefaultConfigPath()
    {
        ...
        return Path.Combine(baseDir, "ModManager", "ai-settings.json");
    }
}
