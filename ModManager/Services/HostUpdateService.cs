...
/// <summary>
/// Prüft GitHub-Releases des Hosts (Kroste/Mod-Manager) auf neue Versionen und
...
public sealed class HostUpdateService
{
    ...
    private const string Repo = "Mod-Manager";
    // Namensschema des Assembly/Exe (weicht vom Repo-Slug ab).
    private const string ExeName = "ModManager";
...
";
    ...
    private static string ResolveWritableLogPath()
    {
        ...
        if (!string.IsNullOrEmpty(xdg))
            return Path.Combine(xdg, "ModManager", "update.log");
        string home = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        if (!string.IsNullOrEmpty(home))
            return Path.Combine(home, ".local", "state", "ModManager", "update.log");
